...
const Navbar: React.FC = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { id: "demo", name: "Live Demo", path: "/demo" },
  ];
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] px-10 py-4 flex justify-between items-center border-b border-[#00FF9C15] backdrop-blur-xl bg-black/80">
      <div
        className=" font-extrabold text-lg text-[#00FF9C] tracking-wider animate-glow cursor-pointer"
            ...
            <span
              ...
              className={`text-[13px] cursor-pointer transition-all duration-200 tracking-wider relative group ${
                activeSection === item.id
                  ? "text-[#00FF9C]"
                  : "text-[#445] hover:text-[#00FF9C]"
              }`}
            >
              {item.name}
              {/* Active indicator */}
              <span
                className={`absolute -bottom-1 left-0 right-0 h-0.5 bg-[#00FF9C] transform origin-left transition-transform duration-300 ${
                  activeSection === item.id
                    ? "scale-x-100"
      ...
      >
        <div
          className={`w-6 h-0.5 bg-[#00FF9C] transition-all duration-300 ${isMenuOpen ? "rotate-45 translate-y-2" : ""}`}
        />
        <div
          className={`w-6 h-0.5 bg-[#00FF9C] transition-all duration-300 ${isMenuOpen ? "opacity-0" : ""}`}
        />
        <div
          className={`w-6 h-0.5 bg-[#00FF9C] transition-all duration-300 ${isMenuOpen ? "-rotate-45 -translate-y-2" : ""}`}
        />
